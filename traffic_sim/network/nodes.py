...
class Junction(Node):
    def __init__(self, node_id, processing_delay=1.0): # Default 1 sec delay
...
class Sink(Node):
    def __init__(self, node_id):
        super().__init__(node_id)
        self.received = []
        # NEW: Initialize the list to store exact wait times
        self.completed_wait_times = []

    def accept_vehicle(self, vehicle):
        self.received.append(vehicle)
        # NEW: Harvest the exact wait time before the vehicle is removed
